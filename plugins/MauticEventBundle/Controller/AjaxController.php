...
class AjaxController extends CommonAjaxController
{
    ...
    protected function fieldListAction(Request $request)
    {
        ...
        $shouldLookup = in_array($field->getAlias(), ['city', 'event', 'title']);
        ...
        if ('event' === $field->getObject()) {
            $results = $eventModel->getLookupResults('eventfield', [$fieldAlias, $filter]);
            foreach ($results as $r) {
     ...
     * @return \Symfony\Component\HttpFoundation\JsonResponse
     */
    protected function toggleEventLeadAction(Request $request)
    {
        $dataArray = ['success' => 0];
        $leadId    = (int) $request->request->get('leadId');
        $eventId = (int) $request->request->get('eventId');
        $action    = InputHelper::clean($request->request->get('eventAction'));

        if (!empty($leadId) && !empty($eventId) && in_array($action, ['remove', 'add'])) {
            $leadModel    = $this->getModel('lead');
            $eventModel = $this->getModel('lead.event');
            ...
            if (null !== $lead && null !== $event) {
                $class = 'add' == $action ? 'addLeadToEvent' : 'removeLeadFromEvent';
                $eventModel->$class($event, $lead);
                $dataArray['success'] = 1;
            }
        }
     ...
     * @return \Symfony\Component\HttpFoundation\JsonResponse
     */
    protected function setAsPrimaryEventAction(Request $request)
    {
        $dataArray['success'] = 1;
        $eventId            = InputHelper::clean($request->request->get('eventId'));
        $leadId               = InputHelper::clean($request->request->get('leadId'));

        $leadModel      = $this->getModel('lead');
        $primaryEvent = $leadModel->setPrimaryEvent($eventId, $leadId);

        $dataArray = array_merge($dataArray, $primaryEvent);
