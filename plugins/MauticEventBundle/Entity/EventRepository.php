<?php
	...
	class EventRepository extends CommonRepository implements CustomFieldRepositoryInterface
	{
		...
		public function getEventsByLeadId($leadId, $eventId = null)
		{
			...
			$q->select('e.id, e.eventname, e.eventcity, e.eventcountry, e.is_primary')
				->from(MAUTIC_TABLE_PREFIX . 'events', 'e')
				->leftJoin('e', MAUTIC_TABLE_PREFIX . 'events_leads', 'e', 'cl.event_id = e.id')
				->where('e.lead_id = :leadId')
				->setParameter('leadId', $leadId)
				->orderBy('e.is_primary', 'DESC');
		...
		public function getEventsByMeeyId($meeyId)
		{
			...
			$q->where($q->expr()->eq('e.eventmeeyid', ':meeyid'))
		 ...
		 * @return int
		 */
		public function getEventsCount($meeyId, array $filters = null)
		{
			$query = $this->_em->getConnection()->createQueryBuilder()
				->from(MAUTIC_TABLE_PREFIX . 'events', 'e')
				->select('count(*)')
				->where('e.eventmeeyid = :meeyid')
				->setParameter('meeyid', $meeyId);
			
			if (is_array($filters) && !empty($filters['search'])) {
				$query->andWhere('e.eventname like \'%' . $filters['search'] . '%\'');
			}
		 ...
		 * @return array
		 */
		public function getEventsPaging($meeyId, array $filters = null, array $orderBy = null, $page = 1, $limit = 25)
		{
			$conn = $this->getEntityManager()->getConnection();
			$q = $conn->createQueryBuilder();
			
			$q->select('e.eventname', 'e.eventmeeyid', 'e.eventplatform', 'e.date_added', 'e.created_by')
				->from('events', 'e')
				->where('e.eventmeeyid = :meeyid')
				->setParameter('meeyid', $meeyId);
			
			if (is_array($filters) && !empty($filters['search'])) {
				$q->andWhere('e.eventname LIKE :search')
