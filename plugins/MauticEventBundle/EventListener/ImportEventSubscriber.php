...
final class ImportEventSubscriber implements EventSubscriberInterface
{
    ...
    {
        if ($event->importIsForRouteObject('events')) {
            if (!$this->corePermissions->isGranted('lead:imports:create')) {
                throw new AccessDeniedException('You do not have permission to import events');
            }

            $event->objectSingular = 'event';
            $event->objectName     = 'mautic.lead.lead.events';
            $event->activeLink     = '#mautic_event_index';
            $event->setIndexRoute('mautic_event_index');
    ...
    public function onFieldMapping(ImportMappingEvent $event): void
    {
        if ($event->importIsForRouteObject('events')) {
            ...
            $event->fields = [
                'mautic.lead.event'        => $this->fieldList->getFieldList(false, false, ['isPublished' => true, 'object' => 'event']),
    ...
    {
        if ($event->importIsForObject('event')) {
            $merged = $this->eventModel->import(
    ...
    {
        if (false === $event->importIsForRouteObject('events')) {
            return;
        }
