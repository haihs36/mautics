...
Mautic.eventOnLoad = function (container, response) {
    ...
    if (mQuery(container + ' #list-search').length) {
        Mautic.activateSearchAutocomplete('list-search', 'lead.event');
    }
}
Mautic.leadOnLoad = function (container, response) {
    ...
    if (mQuery(container + ' .panel-events').length) {
        mQuery(container + ' .panel-events .fa-check').tooltip({html: true});
    }
...
Mautic.toggleEventLead = function(toggleId, leadId, eventId) {
    var action = mQuery('#' + toggleId).hasClass('fa-toggle-on') ? 'remove' : 'add';
    var query = "action=lead:toggleEventLead&leadId=" + leadId + "&eventId=" + eventId + "&eventAction=" + action;
    Mautic.toggleLeadSwitch(toggleId, query, action);
};
...
Mautic.setAsPrimaryEvent = function (eventId,leadId){
    Mautic.ajaxActionRequest('lead:setAsPrimaryEvent', {'eventId': eventId, 'leadId': leadId}, function(response) {
        if (response.success) {
            if (response.oldPrimary == response.newPrimary && mQuery('#event-' + response.oldPrimary).hasClass('primary')) {
                mQuery('#event-' + response.oldPrimary).removeClass('primary');
            } else {
                mQuery('#event-' + response.oldPrimary).removeClass('primary');
                mQuery('#event-' + response.newPrimary).addClass('primary');
            }
