...
use MauticPlugin\MauticTransactionBundle\Entity\Import;
...
class ImportController extends FormController
{
    ...
    public function newAction($objectId = 0, $ignorePost = false)
    {
        ...
        $action = $this->generateUrl('mautic_transaction_import_action', ['object' => $this->request->get('object'), 'objectAction' => 'new']);
        ...
        if (self::STEP_UPLOAD_CSV === $step || self::STEP_MATCH_FIELDS === $step) {
            $contentTemplate = 'MauticTransactionBundle:Import:new.html.php';
            $viewParameters = [
                'form' => $form->createView(),
                'objectName' => $initEvent->objectName,
            ];
        } else {
            $contentTemplate = 'MauticTransactionBundle:Import:progress.html.php';
            $viewParameters = [
                'progress' => $progress,
                'import' => $import,
                'complete' => $complete,
    ...
    protected function generateUrl(string $route, array $parameters = [], int $referenceType = UrlGeneratorInterface::ABSOLUTE_PATH): string
    {
        if (!isset($parameters['object'])) {
            $parameters['object'] = $this->request->get('object', 'transactions');
        }
